...
#    still_linked = R < 10*L0
    still_linked = 1 #dummy this term out
    omega2 = k/m
    ...
    DX2 = -G*(M*X/R + m * UgX) + -omega2*(2*X - (Xm + Xp) - L0*((X-Xm)/Lm + (X-Xp)/Lp))*still_linked
    DY2 = -G*(M*Y/R + m * UgY) + -omega2*(2*Y - (Ym + Yp) - L0*((Y-Ym)/Lm + (Y-Yp)/Lp))*still_linked
    DZ2 = -G*(M*Z/R + m * UgZ) + -omega2*(2*Z - (Zm + Zp) - L0*((Z-Zm)/Lm + (Z-Zp)/Lp))*still_linked
    A = np.concatenate((DX2, DY2, DZ2), axis = 1)
    return A

def motion(XYZ, VEL):
    not_in_star = (abs(XYZ) > 1e-10).any(1, keepdims = True)
    XYZ = XYZ + t_step*VEL
    VEL = VEL + t_step*lagrangian(POS)
    XYZ = XYZ*not_in_star
...
#         point.set_alpha(1)
    for index, line in enumerate(lines):
#        if 10* L0[index,0] > np.linalg.norm(node_distances[index]):
